...
function isAnnotationRenderable(annotation, intent, annotationsNotRendered = []) {
  return (intent === "display" && annotation.viewable || intent === "print" && annotation.printable) && !isAnnotationNotRendered(annotation, annotationsNotRendered);
}

function isAnnotationNotRendered(annotation, annotationsNotRendered) {
  if (!annotation || !annotation.data || !annotation.data.annotationType || !Array.isArray(annotationsNotRendered)) {
    return false;
  }
  ...
  return annotationsNotRendered.some(itm => {
    if (typeof itm === 'object') {
      ...
      for (const k in itm) {
        if (!data.hasOwnProperty(k) || typeof data[k] === 'function' || typeof itm[k] === 'function') {
          remove = false;
        } else if (remove) {
          remove = data[k] === itm[k];
        }
